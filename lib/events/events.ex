defmodule Wargear.Events do
  ...
  defp get_type(event) do
    case event do
      ...
      {_, _, [_, " fortified " <> _rest]} ->
        :unit_fortify

      {_, _, [_, " placed " <> _rest]} ->
        :unit_place

      {_, _, [_, " received " <> _rest]} ->
        :unit_receive

      {_, _, [_, " transferred " <> _rest]} ->
        :unit_transfer
  ...
  defp get_sides({_, _, [{_, _, [attacker]}, " attacked ", {_, _, [defender]}, _]}),
    do: {attacker, defender}

  defp get_sides({_, _, [{_, _, [attacker]}, " attacked ", _, _]}), do: {attacker, nil}

  defp get_sides({_, _, [{_, _, [attacker]}, " occupied ", {_, _, [defender]}, _]}),
    do: {attacker, defender}
